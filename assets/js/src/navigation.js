 ...
 * navigation support for dropdown_nav menus.
 */
...
function initEachNavToggleSubmenu( nav ) {
	...
	// Create the dropdown_nav button.
	const dropdown_navButton = getdropdown_navButton();

	for ( let i = 0; i < SUBMENUS.length; i++ ) {
		const parentMenuItem = SUBMENUS[ i ].parentNode;
		let dropdown_nav = parentMenuItem.querySelector( '.dropdown_nav' );

		// If no dropdown_nav, create one.
		if ( ! dropdown_nav ) {
			// Create dropdown_nav.
			dropdown_nav = document.createElement( 'span' );
			dropdown_nav.classList.add( 'dropdown_nav' );

			const dropdown_navSymbol = document.createElement( 'i' );
			dropdown_navSymbol.classList.add( 'dropdown_nav-symbol' );
			dropdown_nav.appendChild( dropdown_navSymbol );

			// Add before submenu.
			SUBMENUS[ i ].parentNode.insertBefore( dropdown_nav, SUBMENUS[ i ] );
		}

		// Convert dropdown_nav to button.
		const thisdropdown_navButton = dropdown_navButton.cloneNode( true );

		// Copy contents of dropdown_nav into button.
		thisdropdown_navButton.innerHTML = dropdown_nav.innerHTML;

		// Replace dropdown_nav with toggle button.
		dropdown_nav.parentNode.replaceChild( thisdropdown_navButton, dropdown_nav );

		// Toggle the submenu when we click the dropdown_nav button.
		thisdropdown_navButton.addEventListener( 'click', ( e ) => {
...
function toggleSubMenu( parentMenuItem, forceToggle ) {
	const toggleButton = parentMenuItem.querySelector( '.dropdown_nav-toggle' ),
...
/**
 * Returns the dropdown_nav button
 * element needed for the menu.
 * @return {Object} drop-down button element
 */
function getdropdown_navButton() {
	const dropdown_navButton = document.createElement( 'button' );
	dropdown_navButton.classList.add( 'dropdown_nav-toggle' );
	dropdown_navButton.setAttribute( 'aria-expanded', 'false' );
	dropdown_navButton.setAttribute( 'aria-label', wpRigScreenReaderText.expand );
	return dropdown_navButton;
}
